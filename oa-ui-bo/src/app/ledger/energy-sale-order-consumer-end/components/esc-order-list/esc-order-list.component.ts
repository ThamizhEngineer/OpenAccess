...
import { EsOrder } from '../../../energy-sale-order/components/services/esorder';
...
@Component({
  selector: 'esc-order-list',
  templateUrl: './esc-order-list.component.html',
  styleUrls: ['./esc-order-list.component.scss'],
  providers: [CommonUtils]
})
export class EscOrderListComponent implements OnInit {
  ...
  editEso(_id: string) {
    try {
      console.log(_id);
      this.router.navigateByUrl('/esc-order/esc-order-detail/' + _id);
      // to-do
