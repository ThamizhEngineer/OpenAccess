...
@Component({
  selector: 'master-wind-report',
  templateUrl: './master-wind-report.component.html',
})
export class MasterWindReportComponent implements OnInit {
...
statusList=[
  ...
  searchResults(){
    ...
    genRptInput.reportName = 'MASTER WIND REPORT';
...
fetchDist(){
  ...
  openDownload(data: Response) {
    ...
    let x_filename = "master-wind-report.pdf";
    console.log(content_type)
    saveAs(data.blob(), x_filename);
  }

  printReport(){
    this.service.printMasterWind(this.Orgid,this.Year,this.flowType,this.substation,this.isRec,this.District,this.status).subscribe(xs=>{
      this.printdata=xs;
  ...
  exportAsXLSX():any {
    ...
    str = str.replace(/\"op6\":/g, "\"NO OF UNITS\":");
    str = str.replace(/\"op7\":/g, "\"CAPACITY\":");
    str = str.replace(/\"op8\":/g, "\"TOTAL CAPACITY\":");
...
if(this.flowType=='STB'&& this.flowType!=undefined){
  ...
  str = str.replace(/\"op20\":/g, "\"MODEM NUMBER\":");
    ...
    this.excelService.exportAsExcelFile(this.rows,'master-wind-report');
    this.searchResults();
}
