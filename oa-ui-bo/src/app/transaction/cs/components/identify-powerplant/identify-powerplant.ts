...
export class IdentifyPowerplant implements OnInit {
  ...
  filterSellerCompanyServiceList(val: string) {
    ...
    return val ? this.sellerCompanyServices.filter((s) => s.serviceNumber.includes(val)) : this.sellerCompanyServices;
  }
  ...
  onServiceChange(){
    ...
    if(this.filteredCompanyServiceList.length == 1){
      this.cs.sellerCompanyServiceNumber  = this.filteredCompanyServiceList[0].serviceNumber;
    
      this.cs.sellerCompanyId =this.filteredCompanyServiceList[0].companyId;

      this.cs.sellerCompanyServiceId= this.filteredCompanyServiceList[0].serviceId;
    ...
    // console.log(this.cs.sellerCompanyName)
