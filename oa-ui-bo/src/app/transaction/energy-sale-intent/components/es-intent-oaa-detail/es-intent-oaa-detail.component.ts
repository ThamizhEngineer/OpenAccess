...
@Component({
  selector: 'es-intent-oaa-detail',
  templateUrl: './es-intent-oaa-detail.component.html',
 // styleUrls: [],
  providers: [OaaService,MatDatepickerModule, MatNativeDateModule, DatePipe ,CommonUtils,EsIntentService,
    {
...
export class EsIntentOaaDetailComponent implements OnInit{ 
...
this.esIntent = new EsIntent();
    if(this.route.params['_value']['_id']){
      ...
      this.route.params
          ...
          this.esIntentService.getEsIntentById(this.oaa.tEsIntentId).subscribe(_esIntent=>{
  ...
  fetchIntervalTypeCode() {
    ...
    onAdd() {
        ...
        if(this.oaa.intervalTypeCode=="01"){
          ...
          total = (parseFloat((this.oaa.peakUnits)) + parseFloat(this.oaa.offPeakUnits));
          ...
          total = (parseFloat(this.oaa.c1Units) + parseFloat(this.oaa.c2Units) + parseFloat(this.oaa.c3Units) + parseFloat(this.oaa.c4Units) + parseFloat(this.oaa.c5Units));
        ...
        if(total>parseFloat(this.esIntent.ewaTotalApprovedCapacity)){
          alert("Quantum cannot be more than Wheeling Agreement capacity");
        }else{
 ...
 setNextStatus(status: string){
  ...
  formatChangesforDB() {
    ...
    this.oaa.createdDate = this.datePipe.transform(this.oaa.createdDate, 'yyyy-MM-dd');
    this.oaa.modifiedDate = this.datePipe.transform(this.oaa.modifiedDate, 'yyyy-MM-dd');
    ...
    this.oaa.agreementDate = this.datePipe.transform(this.oaa.agreementDate, 'yyyy-MM-dd');
    this.oaa.approvedDate = this.datePipe.transform(this.oaa.approvedDate, 'yyyy-MM-dd');
    this.oaa.appliedDate = this.datePipe.transform(this.oaa.appliedDate, 'yyyy-MM-dd');
  ...
  listOaas() {
    this.router.navigateByUrl('/es-intent/es-intent-oaa-list/'+this.oaa.tEsIntentId);
  }
