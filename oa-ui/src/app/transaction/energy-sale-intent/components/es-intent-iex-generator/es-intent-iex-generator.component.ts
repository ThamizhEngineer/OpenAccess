...
@Component({
  selector: 'es-intent-iex-generator',
  templateUrl: './es-intent-iex-generator.component.html',
  providers: [EsIntentService, MatDatepickerModule,  MatNativeDateModule, DatePipe
    ,CommonService
  ]
})
export class EsIntentIexGeneratorComponent implements OnInit {
  esIntent:EsIntent;
  ...
  onEdcChange(){
    ...
    this.esIntentLine.buyerCompanyServiceId= "IEX-"+this.esIntentLine.buyerEndOrgId;
  ...
  onBuyerServiceChange(){   
    var sellerCompanyService = this.sellerCompanyServices.filter((item)=> item.id == this.esIntentLine.buyerCompanyServiceId)[0];
