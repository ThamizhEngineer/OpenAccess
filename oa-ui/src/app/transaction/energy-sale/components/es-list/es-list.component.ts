...
export class EsListComponent implements OnInit {
  ...
  stopFlow:boolean = false;// stop allotments if this message has value
  stopMessage = ""; 
...
this.searchResults();
 ...
 //when we need generator first name for login use this eg: Premier cotton textiles login
  ...
  if(user.loginStopMessage == null || user.loginStopMessage ==''){
    this.stopFlow = false;
    this.stopMessage ="";
  }
  else{
    this.accessAddFlag = false;
    this.stopFlow = true;
    this.stopMessage = user.loginStopMessage;
  ...
  searchResults() {
    ...
    this.service.getAllEss(this.searchSellerCompanyName, this.sellerCompanyServiceNumber,this.searchOrgId,this.searchSellerCompanyID,this.searchIsSTB, this.searchMonth, this.searchYear, 'N','').subscribe(
      _es => {
        this.rows = _es;
