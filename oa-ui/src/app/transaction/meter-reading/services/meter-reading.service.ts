...
export class MeterReadingService {
    ...
    // }
        ...
        return this.http.get(environment.serviceApiUrl+'/api/meterreading/'+_id,new CustomRequestOptions()).map(res => res.json());
    }
    
     getMeterReading(companyServiceNumber:string,modemNumber:string,meterNumber:string,month:string,year:string,orgId:string,companyName:string){
        
        var url = environment.serviceApiUrl+'/transaction/meterreadings';
        if(companyServiceNumber!= "" || modemNumber != "" || meterNumber != "" || month != "" || year!="" || orgId!="" || companyName!="" ) {
            url = url + "?dummy=1";  
