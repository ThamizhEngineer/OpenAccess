...
@Service
public class SolarFeederEdcWiseReport {
    
    @Value("${file.location}")
    private String fileLocation;
    
    @Value("${solar-feeder-edc-wise.url}")
    ...
    private  final String TEMPLATE_NAME = "solarfeeder-edcwise";
...
public Context setContext(List<GenericReportOutput> solarfeederedcReport) {
    final Context ctx = new Context(Locale.ENGLISH);
    
    
    try {
        System.out.println("in context--");
        System.out.println("test"+solarfeederedcReport);

        ctx.setVariable("solarfeederedcReport", solarfeederedcReport);
        
        String month="";
        String year="";
      for(GenericReportOutput solarfeeder:solarfeederedcReport) {
    	  month=solarfeeder.getOp9();
    	  year=solarfeeder.getOp10();
      }
      System.out.println("came------"+month);
      System.out.println("came2------"+year);
