...
import com.ss.oa.oadocumentservice.vo.NilGenerationReport;
...
public class NilGenerationReportPrinter {
	...
	@Value("${nil-generation-report.url}")
	...
	private  final String TEMPLATE_NAME = "NilGenerationReport";
...
public List<GenericReportOutput> fetchReportData(PrintPayload pl) {	
		...
		public Context setContext(List<GenericReportOutput> nilGenReportList) {
			final Context ctx = new Context(Locale.ENGLISH);
			
			
			try {
				System.out.println("in context--");
				System.out.println(nilGenReportList);

				ctx.setVariable("nilGenReportList", nilGenReportList);
				double totalop6=0;
				double totalop7=0;
				double totalop8=0;
				double totalop9=0;
				...
				for(GenericReportOutput row:nilGenReportList) {
