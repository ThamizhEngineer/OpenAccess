...
@Service
public class UnutilisedBankingMarNew {

	
	@Value("${file.location}")
	private String fileLocation;
	
	@Value("${unutilised-banking-mar-new.url}")
	...
	private  final String TEMPLATE_NAME = "unutilised-banking-mar-new";
...
public List<GenericReportOutput> fetchReportData(PrintPayload pl) {	
		...
		public Context setContext(List<GenericReportOutput> UnutilisedBankingNew) {
			...
			try {
				...
				for(GenericReportOutput row : UnutilisedBankingNew) {
					cDate=CommonUtils.convertDateFormat(row.getOp4(), "DD/MM/YYYY");
				row.setOp4(cDate);
				...
				ctx.setVariable("UnutilisedBankingNew", UnutilisedBankingNew);
