CREATE OR REPLACE PACKAGE BODY OPENACCESS."IMP_INT_ADJUSTED_UNIT" 
AS
-- Package body
...
BEGIN
 ...
 PROCEDURE DIRECT_ADJUSTMENT_FROMHT(from_date IN varchar2,to_datea IN varchar2,output1 OUT VARCHAR2,output2 OUT VARCHAR2) IS
	v_process varchar2(50):='IMP_INT_ADJUSTED_UNIT.DIRECT_ADJUSTMENT_FROMHT';
	v_process_type varchar2(500):='PROCEDURE';
    v_batch_key varchar2(500);
    v_batch_key_id varchar2(500);
    reading_mnth  varchar2(500);
    reading_yr varchar2(500);
    v_stage varchar2(500):='';
	v_step varchar2(500):='';
    v_message varchar2(500):='';
    v_log_result  varchar2(50);
    v_input varchar(500) :='';
   v_exception_code varchar(500) :='';
  v_exception_msg varchar(500) :='';
  from_date_shed varchar(500) ;
    to_date_shed varchar(500);
    int_id varchar(500) ;
BEGIN
    BEGIN --EXCEPTION-HANDLER BLOCK STARTS HERE    
        v_stage:='INIT'; v_step:='START'; v_message:=v_input;
		v_log_result := log_activity(v_process_type,v_process,v_stage,v_step,to_datea,from_date,'started',sysdate);
	if(from_date IS NULL OR to_datea IS null)THEN
	v_log_result := log_activity(v_process_type,v_process,v_stage,v_step,v_message,v_process,'fromdate and todate should not be null',sysdate);
else
  IF(FROM_DATE = 'SHEDULER' AND TO_DATEA ='SHEDULER')THEN 
  v_log_result := log_activity(v_process_type,v_process,v_stage,v_step,from_date_shed,to_date_shed,'started sheduler',sysdate);
    SELECT INT_ADJUSTMENT_BATCH_KEY.nextval INTO v_batch_key_id from dual;
    SELECT INT_ADJUSTMENT_ID.nextval INTO int_id from dual;
    v_batch_key:='HTDIRECT'||v_batch_key_id;
    FOR ROW IN (
SELECT *
FROM ht_surplus_new
WHERE CREATED_DT BETWEEN SYSDATE - interval '2' HOUR   AND SYSDATE AND substr( SELLERSERVICENO, 3,3)<>'909' AND substr( SELLERSERVICENO , 2,3)<>'909'
...
(ID,
	...
	MODIFIED_DT,
	BUYER_COMPANY_ID,
	BUYER_COMPANY_SERVICE_ID,
	SELLER_COMPANY_ID,
	SELLER_COMPANY_SERVICE_ID,
	FUEL_TYPE_CODE,
	IMPORT_REMARKS,
	"SOURCE",
	IMPORTED,
...
LOOP;
    ...
    for row in (SELECT *
FROM ht_surplus_new
WHERE CREATED_DT BETWEEN TO_TIMESTAMP(from_date,'YY-MM-DD HH24:MI:SS.FF')  AND TO_TIMESTAMP(to_datea,'YY-MM-DD HH24:MI:SS.FF') 
AND substr( SELLERSERVICENO, 3,3)<>'909' AND substr( SELLERSERVICENO , 2,3)<>'909'
 ORDER BY CREATED_DT ASC)
        LOOP
        
        
        INSERT INTO OPENACCESS.INT_ADJUSTED_UNIT
(ID, BATCH_KEY, SERVICE_NO, SUPLR_CODE, READING_DT, READING_MNTH, READING_YR, C24, C1, C2, C3, C4, C5, CREATED_BY, CREATED_DT, MODIFIED_BY, MODIFIED_DT, BUYER_COMPANY_ID, BUYER_COMPANY_SERVICE_ID, SELLER_COMPANY_ID, SELLER_COMPANY_SERVICE_ID, FUEL_TYPE_CODE, IMPORT_REMARKS, "SOURCE", IMPORTED, SUPLR_NAME, DELETE_FLAG, SUPLR_TYPE, C1_WITHLOSS, C2_WITHLOSS, C3_WITHLOSS, C4_WITHLOSS, C5_WITHLOSS, ADJUSTED_C1, ADJUSTED_C2, ADJUSTED_C3, ADJUSTED_C4, ADJUSTED_C5, IS_CLEAN, F_ENERGY_SALE_ORDER_ID, F_ENERGY_SALE_ORDER_LINE_ID, RESULT_CODE, RESULT_NAME, SELLER_ORG_ID, RESULT_DESC)
VALUES(int_id,v_batch_key,ROW.BUYERSERVICENO,ROW.SELLERSERVICENO,ROW.READINGDT, '0'||ROW.READINGMNTH,ROW.READINGYR,ROW.SURPLUSC24,ROW.SURPLUSC1,ROW.SURPLUSC2,ROW.SURPLUSC3,ROW.SURPLUSC4,ROW.SURPLUSC5, '', sysdate, '', '', '', '', '', '', '', '', 'HT','',ROW.SELLERNAME, '', '', 0, 0, 0, 0, 0,ROW.ADJUSTEDC1,ROW.ADJUSTEDC2,ROW.ADJUSTEDC3,ROW.ADJUSTEDC4,ROW.ADJUSTEDC5, '', '', '', '', '', '', '');
       ...
       SELECT DISTINCT (READING_MNTH) INTO reading_mnth FROM INT_ADJUSTED_UNIT WHERE BATCH_KEY = v_batch_key;    
       SELECT DISTINCT (READING_YR) INTO reading_yr FROM INT_ADJUSTED_UNIT WHERE BATCH_KEY = v_batch_key;
       PROCESS_INT_ADJUSTED_UNIT(v_batch_key,reading_mnth,reading_yr,output1,output2);
      v_log_result := log_activity(v_process_type,v_process,'DIRECT HT DATA IMPORTED','','','', sysdate,v_batch_key);
    ...
    when others then
      ...
      v_exception_msg := SUBSTR(SQLERRM, 1, 200);
      output1 := 'FAILURE';
      output2 := v_exception_code || ' - ' || v_exception_msg;
      -- dbms_output.put_line(o_result_desc);
      v_log_result := log_activity(v_process_type,v_process,'EXCEPTION',output1,output2,'', sysdate,v_batch_key);
       

COMMIT;
END;
    END DIRECT_ADJUSTMENT_FROMHT;
