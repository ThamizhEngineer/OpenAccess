CREATE OR REPLACE PACKAGE BODY OPENACCESS."DELETE_TXN" AS
...
BEGIN
    BEGIN
     ...
     if(i_reading_year <> v_billing_year AND i_reading_month <> v_billing_month)then
     
         dbms_output.put_line('Cannot delete previous month and year transcations');
         Raise_Application_Error (-20343, 'Cannot delete previous month and year transcations ');
