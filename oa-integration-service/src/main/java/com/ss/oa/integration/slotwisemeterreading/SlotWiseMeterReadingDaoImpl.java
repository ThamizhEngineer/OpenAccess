...
@Component
public class SlotWiseMeterReadingDaoImpl extends BaseDaoJdbc implements SlotWiseMeterReadingDao {
	...
	public  List<SlotWiseMeterReading> getAllSlotWiseMeterReading(Map<String,String>criteria){
		...
		String sql="SELECT '' ID,slotwise.METERNO,slotwise.MF,slotwise.SERVICENO,to_char(slotwise.SYS_DT,'yyyy-mm-dd') SYS_DT,to_char(slotwise.INITIAL_READING_DATE,'yyyy-mm-dd') INITIAL_READING_DATE,\n" + 
				...
				"                slotwise.EXP_KVAH_INIT,slotwise.IMP_KVAH_FINAL,slotwise.EXP_KVAH_FINAL,slotwise.Q1_KVARH_INIT,slotwise.Q2_KVARH_INIT,slotwise.Q3_KVARH_INIT,slotwise.Q4_KVARH_INIT,slotwise.Q1_KVARH_FINAL,\n" + 
				...
				"                FROM "+mdmsSchemaName+".UV_TBL_HISTORY_SLOT slotwise WHERE 1=1 ";
		...
		return (ArrayList<SlotWiseMeterReading>) jdbcOperations.query(sql,new SlotWiseMeterReadingMapper());		
	}
	...
	String sql="SELECT '' ID,slotwise.METERNO,slotwise.MF,slotwise.SERVICENO,to_char(slotwise.SYS_DT,'yyyy-mm-dd') SYS_DT,to_char(slotwise.INITIAL_READING_DATE,'yyyy-mm-dd') INITIAL_READING_DATE,\n" + 
			...
			"                slotwise.EXP_KVAH_INIT,slotwise.IMP_KVAH_FINAL,slotwise.EXP_KVAH_FINAL,slotwise.Q1_KVARH_INIT,slotwise.Q2_KVARH_INIT,slotwise.Q3_KVARH_INIT,slotwise.Q4_KVARH_INIT,slotwise.Q1_KVARH_FINAL,\n" + 
			"                slotwise.Q2_KVARH_FINAL,slotwise.Q3_KVARH_FINAL,slotwise.Q4_KVARH_FINAL,to_char(slotwise.initial_reading_Date,'mm') MONTH,to_char(slotwise.initial_reading_Date,'YYYY') year \n" + 
			"                FROM TANGEDCO.UV_TBL_HISTORY_SLOT slotwise WHERE slotwise.ID=?";
	
	return jdbcOperations.queryForObject(sql,new Object[]{id},new SlotWiseMeterReadingMapper());
	
   }
	
	final class SlotWiseMeterReadingMapper implements RowMapper<SlotWiseMeterReading>{
		...
		@Override
		public SlotWiseMeterReading mapRow(ResultSet resultSet, int rownum) throws SQLException {
			SlotWiseMeterReading slotWiseMeterReading=new SlotWiseMeterReading();
